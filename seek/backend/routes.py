from flask import jsonify, request, Flask, render_template, send_file,send_from_directory
...
def init_app(app):
    ...
    def login():
        try:
            ...
            role_mapping = db.session.execute(
                db.select(roles_users.c.role_id).where(roles_users.c.user_id == user.user_id)
            ).fetchone()
            ...
            if role_name == "student":
                name = (Student.query.filter_by(user_id=user.user_id).first()).first_name
    ...
    def register():
        try:
            ...
            if role == "admin":
                return jsonify({"message": "Method not allowed! Register as Student or Instructor"}), 400

            # Check if required fields are provided
            if not email or not password or not role:
                return jsonify({"message": "Invalid request format! Required fields(email/password/role) are missing "}), 400
            ...
            if role == "student":
                if not data.get("first_name"):
                    return jsonify({"message": "Invalid request format! 'first_name' is required for students"}), 400
                
                last_name = data.get("last_name") if data.get("last_name") else None
                contact_info = data.get("contact_info") if data.get("contact_info") else None
                ...
                if not data.get("name"):
                    return jsonify({"message": "Invalid request format! 'name' is required for instructors"}), 400
                
                contact_info = data.get("contact_info") if data.get("contact_info") else None
                experience = data.get("experience") if data.get("experience") else "0"
                qualification = data.get("qualification") if data.get("qualification") else "No data"
