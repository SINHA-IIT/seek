...
const routes = [
    ...
    { path: '/instructor-dashboard', component: InstructorDashboard, meta: { requiresLogin: true, role: 'instructor' } },
...
router.beforeEach((to, from, next) => {
    ...
    if (to.meta.requiresLogin && !isLoggedIn) {
        ...
        alert("already login hai");
        next('/dashboard');  // Prevent logged-in users from accessing login page
    } else if (to.meta.role) {
        if (Array.isArray(to.meta.role) ? !to.meta.role.includes(userRole) : to.meta.role !== userRole) {
            alert("Access Denied! You are not authorized to view this page.");
            next(false);  // purane page pr wapis
        } else {
            next();
        }
    } else {
